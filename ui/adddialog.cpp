...
AddDialog::AddDialog(int idScientistToEdit, QWidget *parent) :  QDialog(parent),
    ui(new Ui::AddDialog) {
...
void AddDialog::setUpForEdit() {
    ...
    scientistEdit = (services.searchScientists("?? id=" + getScientistQuery)).at(0);
...
void AddDialog::addToDatabaseConfirm() {
    ...
    confirmationText = QString("Confirm \n\nName: %1\nGender: %2\nBorn: %3\nDied: %4\nKnown for: %5 \n\n"
                               "Are you sure you want to add this entry?")
                               	.arg(name).arg(ui->gendrComboBox->currentText())
                                .arg(ui->bornAddSpinBox->value()).arg(alive).arg(knownFor);
...
void AddDialog::updateDatabaseConfirm() {
    ...
    newScientistInfo = QString("Name: %1\nGender: %2\nBorn: %3\nDied: %4\nKnown for: %5 \n\n")
                       .arg(name).arg(ui->gendrComboBox->currentText()).arg(ui->bornAddSpinBox->value())
                       .arg(alive).arg(knownFor);
    ...
    if (addMsgBox.clickedButton() == pButtonYes) {
        ...
        Scientist updatedScientist(id, nameIs, lastname, gender,
                                   bYear, dYear, knownForIs);
